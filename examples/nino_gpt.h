 ...
 *
 * GPU acceleration: MLP matmuls dispatch to cuBLAS via reshape-to-2D pattern.
 * Embedding, RMSNorm, and Attention use CPU with transparent fallback.
 *
 * Conversation format:
...
// Uses reshape-to-2D so matmul dispatches to cuBLAS on GPU.
// =============================================================================
static TensorPtr linear3d(const TensorPtr& input, Linear* layer) {
    size_t B = input->shape[0];
    size_t S = input->shape[1];
    auto flat = input->reshape({B * S, layer->in_features});    // zero-copy view
    auto out = flat->matmul(layer->weight)->add(layer->bias);   // cuBLAS on GPU
    return out->reshape({B, S, layer->out_features});            // zero-copy view
}

// =============================================================================
// Helper: element-wise SiLU on a 3-D tensor
// Uses framework Tensor::silu() which dispatches to CUDA kernel on GPU.
// =============================================================================
...
// Uses framework Tensor::add() which dispatches to CUDA on GPU.
// =============================================================================
...
struct NinoGPT : public Module {
    ...
    TensorPtr forward(const TensorPtr& tokens) override {
        ...
        // Embedding runs on CPU (indices are CPU), result stays CPU
        ...
        // This is a custom op because pe has fewer dims than te
...
// Operates on CPU; transfers from GPU if needed.
// =============================================================================
static TensorPtr lm_cross_entropy(const TensorPtr& logits, const TensorPtr& targets) {
    // If logits are on GPU, transfer to CPU for loss computation
    TensorPtr cpu_logits = logits;
    TensorPtr cpu_targets = targets;
#if defined(WHITEMATTER_CUDA)
    bool on_gpu = (logits->device == whitematter::DeviceType::CUDA);
    if (on_gpu) {
        cpu_logits = logits->to(whitematter::DeviceType::CPU);
        cpu_targets = targets->to(whitematter::DeviceType::CPU);
    }
#endif

    size_t batch      = cpu_logits->shape[0];
    size_t seq_len    = cpu_logits->shape[1];
    size_t vocab_size = cpu_logits->shape[2];
    size_t N = batch * seq_len;

    auto loss = Tensor::create({1}, cpu_logits->requires_grad);
    loss->data()[0] = 0.0f;
    ...
    for (size_t b = 0; b < batch; b++) {
        for (size_t s = 0; s < seq_len; s++) {
            ...
            for (size_t v = 0; v < vocab_size; v++)
                mx = std::max(mx, cpu_logits->data()[offset + v]);

            float sum_exp = 0.0f;
            for (size_t v = 0; v < vocab_size; v++)
                sum_exp += std::exp(cpu_logits->data()[offset + v] - mx);
            float log_Z = mx + std::log(sum_exp);

            for (size_t v = 0; v < vocab_size; v++)
                log_probs[offset + v] = cpu_logits->data()[offset + v] - log_Z;

            size_t label = static_cast<size_t>(cpu_targets->data()[b * seq_len + s]);
    ...
    if (loss->requires_grad) {
        auto lp = logits;  // original (possibly GPU) logits for backward
        loss->parents = {lp};
        loss->grad_fn = [=]() {
            // Backward always runs on CPU, writes to logits grad
            // If logits are on GPU, we need to transfer grad back
            float scale = loss->grad()[0] / static_cast<float>(N);
#if defined(WHITEMATTER_CUDA)
            if (lp->device == whitematter::DeviceType::CUDA) {
                // Compute gradients on CPU buffer, then copy to GPU
                std::vector<float> grad_buf(batch * seq_len * vocab_size, 0.0f);
                for (size_t b = 0; b < batch; b++) {
                    for (size_t s = 0; s < seq_len; s++) {
                        size_t offset = (b * seq_len + s) * vocab_size;
                        size_t label = static_cast<size_t>(cpu_targets->data()[b * seq_len + s]);
                        for (size_t v = 0; v < vocab_size; v++) {
                            ...
                            float g = softmax_v;
                            if (v == label) g -= 1.0f;
                            grad_buf[offset + v] = scale * g;
                        }
                    }
                }
                // Add gradients to GPU tensor
                auto& be = whitematter::CUDABackend::instance();
                std::vector<float> existing(lp->size());
                be.memcpy_d2h(existing.data(), lp->grad(), lp->size());
                for (size_t i = 0; i < lp->size(); i++)
                    existing[i] += grad_buf[i];
                be.memcpy_h2d(lp->grad(), existing.data(), lp->size());
            } else
#endif
            {
                for (size_t b = 0; b < batch; b++) {
...
// Always runs on CPU for simplicity (inference is fast enough).
// =============================================================================
static std::string generate(NinoGPT& model, const std::string& context,
    ...
    for (size_t t = 0; t < max_tokens; t++) {
        ...
        // Ensure logits are on CPU for sampling
        TensorPtr cpu_logits = logits;
#if defined(WHITEMATTER_CUDA)
        if (logits->device == whitematter::DeviceType::CUDA)
            cpu_logits = logits->to(whitematter::DeviceType::CPU);
#endif
        ...
        float sum = 0.0f;
        for (size_t v = 0; v < VOCAB_SIZE; v++) {
            probs[v] = std::exp((cpu_logits->data()[offset + v] - mx) / temperature);
            sum += probs[v];
        }
        for (size_t v = 0; v < VOCAB_SIZE; v++)
            probs[v] /= sum;
