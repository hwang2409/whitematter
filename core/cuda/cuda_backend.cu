...
// Cross-entropy backward: fused softmax gradient
__global__ void cross_entropy_bwd_k(const float* logits, const float* targets,
                                     float* grad_logits, int batch, int classes,
                                     float upstream_grad) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= batch * classes) return;
    int i = idx / classes, j = idx % classes;

    // Compute softmax inline for numerical stability
    const float* row = logits + i * classes;
    float max_val = row[0];
    for (int c = 1; c < classes; c++) max_val = fmaxf(max_val, row[c]);
    float sum_exp = 0.0f;
    for (int c = 0; c < classes; c++) sum_exp += expf(row[c] - max_val);
    float softmax_j = expf(row[j] - max_val) / sum_exp;

    int target = (int)targets[i];
    grad_logits[idx] += (softmax_j - (j == target ? 1.0f : 0.0f)) * upstream_grad / (float)batch;
}

// Global average pooling backward (output_h=1, output_w=1)
__global__ void global_avgpool_bwd_k(const float* grad_out, float* grad_in,
                                      int total, int spatial) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= total) return;
    int bc = idx / spatial;
    grad_in[idx] += grad_out[bc] / (float)spatial;
}

// Elementwise accumulate: target[i] += source[i]
__global__ void accumulate_k(float* target, const float* source, int n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
// ---------------------------------------------------------------------------
// Host-pointer matmul backward: grad_A += grad_C @ B^T, grad_B += A^T @ grad_C
// Uses cuBLAS with row-major trick. Accumulates into existing gradients.
// ---------------------------------------------------------------------------
void CUDABackend::matmul_backward_host(const float* h_grad_C, const float* h_A, const float* h_B,
    ...
    size_t sGC = (size_t)M * N, sA = (size_t)M * K, sB = (size_t)K * N;

    // Check shadow cache for inputs
    float* d_grad_C = g_shadow_cache.find(h_grad_C);
    float* d_A = g_shadow_cache.find(h_A);
    float* d_B = g_shadow_cache.find(h_B);
    bool gc_shadow = (d_grad_C != nullptr);
    bool a_shadow = (d_A != nullptr);
    bool b_shadow = (d_B != nullptr);

    float* p_gc = nullptr;
    float* p_a = nullptr;
    float* p_b = nullptr;

    // Upload inputs not in shadow cache
    if (!gc_shadow) {
        d_grad_C = g_buf_cache.get(sGC);
        p_gc = g_pinned_cache.get(sGC);
        memcpy(p_gc, h_grad_C, sGC * sizeof(float));
        CUDA_CHECK(cudaMemcpyAsync(d_grad_C, p_gc, sGC * sizeof(float),
                                    cudaMemcpyHostToDevice, stream));
    }
    if (!a_shadow) {
        d_A = g_buf_cache.get(sA);
        p_a = g_pinned_cache.get(sA);
        memcpy(p_a, h_A, sA * sizeof(float));
        CUDA_CHECK(cudaMemcpyAsync(d_A, p_a, sA * sizeof(float),
                                    cudaMemcpyHostToDevice, stream));
    }
    if (!b_shadow) {
        d_B = g_buf_cache.get(sB);
        p_b = g_pinned_cache.get(sB);
        memcpy(p_b, h_B, sB * sizeof(float));
        CUDA_CHECK(cudaMemcpyAsync(d_B, p_b, sB * sizeof(float),
                                    cudaMemcpyHostToDevice, stream));
    }

    float alpha = 1.0f, beta_one = 1.0f;

    // grad_A += grad_C @ B^T  (row-major via column-major trick)
    if (h_grad_A) {
        float* d_grad_A = g_buf_cache.get(sA);
        float* pa = g_pinned_cache.get(sA);
        memcpy(pa, h_grad_A, sA * sizeof(float));
        CUDA_CHECK(cudaMemcpyAsync(d_grad_A, pa, sA * sizeof(float),
                                    cudaMemcpyHostToDevice, stream));

        cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N,
                    K, M, N, &alpha, d_B, N, d_grad_C, N, &beta_one, d_grad_A, K);

        CUDA_CHECK(cudaMemcpyAsync(pa, d_grad_A, sA * sizeof(float),
                                    cudaMemcpyDeviceToHost, stream));
        CUDA_CHECK(cudaStreamSynchronize(stream));
        memcpy(h_grad_A, pa, sA * sizeof(float));
        g_pinned_cache.put(pa, sA);
        g_buf_cache.put(d_grad_A, sA);
    }

    // grad_B += A^T @ grad_C  (row-major via column-major trick)
    if (h_grad_B) {
        float* d_grad_B = g_buf_cache.get(sB);
        float* pb = g_pinned_cache.get(sB);
        memcpy(pb, h_grad_B, sB * sizeof(float));
        CUDA_CHECK(cudaMemcpyAsync(d_grad_B, pb, sB * sizeof(float),
                                    cudaMemcpyHostToDevice, stream));

        cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T,
                    N, K, M, &alpha, d_grad_C, N, d_A, K, &beta_one, d_grad_B, N);

        CUDA_CHECK(cudaMemcpyAsync(pb, d_grad_B, sB * sizeof(float),
                                    cudaMemcpyDeviceToHost, stream));
        CUDA_CHECK(cudaStreamSynchronize(stream));
        memcpy(h_grad_B, pb, sB * sizeof(float));
        g_pinned_cache.put(pb, sB);
        g_buf_cache.put(d_grad_B, sB);
    }

    // Cleanup non-shadow inputs
    if (p_gc) g_pinned_cache.put(p_gc, sGC);
    if (p_a) g_pinned_cache.put(p_a, sA);
    if (p_b) g_pinned_cache.put(p_b, sB);
    if (!gc_shadow) g_buf_cache.put(d_grad_C, sGC);
    if (!a_shadow) g_buf_cache.put(d_A, sA);
    if (!b_shadow) g_buf_cache.put(d_B, sB);
}

// ---------------------------------------------------------------------------
// Host-pointer cross-entropy backward: fused softmax gradient
// Accumulates: grad_logits[i] += ...
// ---------------------------------------------------------------------------
void CUDABackend::cross_entropy_backward_host(const float* h_logits, const float* h_targets,
                                               float* h_grad_logits, size_t batch, size_t num_classes,
                                               float upstream_grad) {
    init();
    if (!initialized_ || batch == 0 || num_classes == 0) return;
    ...
    bool logits_shadow = (d_logits != nullptr);

    float* d_targets = g_shadow_cache.find(h_targets);
    bool targets_shadow = (d_targets != nullptr);

    float* p_logits = nullptr;
    float* p_targets = nullptr;

    if (!logits_shadow) {
        d_logits = g_buf_cache.get(total);
        p_logits = g_pinned_cache.get(total);
        memcpy(p_logits, h_logits, total * sizeof(float));
        CUDA_CHECK(cudaMemcpyAsync(d_logits, p_logits, total * sizeof(float),
                                    cudaMemcpyHostToDevice, stream));
    }
    if (!targets_shadow) {
        d_targets = g_buf_cache.get(batch);
        p_targets = g_pinned_cache.get(batch);
        memcpy(p_targets, h_targets, batch * sizeof(float));
        CUDA_CHECK(cudaMemcpyAsync(d_targets, p_targets, batch * sizeof(float),
                                    cudaMemcpyHostToDevice, stream));
    }

    // Upload existing grad_logits for accumulation
    float* d_grad_logits = g_buf_cache.get(total);
    float* p_grad = g_pinned_cache.get(total);
    memcpy(p_grad, h_grad_logits, total * sizeof(float));
    CUDA_CHECK(cudaMemcpyAsync(d_grad_logits, p_grad, total * sizeof(float),
                                cudaMemcpyHostToDevice, stream));

    // Launch kernel
    cross_entropy_bwd_k<<<grid_for((int)total), kBlock, 0, stream>>>(
        d_logits, d_targets, d_grad_logits, (int)batch, (int)num_classes, upstream_grad);

    // D2H
    CUDA_CHECK(cudaMemcpyAsync(p_grad, d_grad_logits, total * sizeof(float),
                                cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    memcpy(h_grad_logits, p_grad, total * sizeof(float));

    // Return buffers
    g_pinned_cache.put(p_grad, total);
    if (p_logits) g_pinned_cache.put(p_logits, total);
    if (p_targets) g_pinned_cache.put(p_targets, batch);

    g_buf_cache.put(d_grad_logits, total);
    if (!logits_shadow) g_buf_cache.put(d_logits, total);
    if (!targets_shadow) g_buf_cache.put(d_targets, batch);
}

// ---------------------------------------------------------------------------
// Host-pointer adaptive average pool backward (global avg pool: spatial -> 1x1)
// Accumulates: grad_in[idx] += grad_out[bc] / spatial
// ---------------------------------------------------------------------------
void CUDABackend::adaptive_avgpool_backward_host(const float* h_grad_out, float* h_grad_in,
                                                  size_t batch, size_t channels, size_t spatial) {
    init();
    if (!initialized_ || batch == 0 || channels == 0 || spatial == 0) return;

    cudaStream_t stream = static_cast<cudaStream_t>(stream_);
    size_t bc = batch * channels;
    size_t total = bc * spatial;

    // Check shadow cache for grad_out
    float* d_grad_out = g_shadow_cache.find(h_grad_out);
    bool grad_out_shadow = (d_grad_out != nullptr);

    float* p_grad_out = nullptr;

    if (!grad_out_shadow) {
        d_grad_out = g_buf_cache.get(bc);
        p_grad_out = g_pinned_cache.get(bc);
        memcpy(p_grad_out, h_grad_out, bc * sizeof(float));
        CUDA_CHECK(cudaMemcpyAsync(d_grad_out, p_grad_out, bc * sizeof(float),
                                    cudaMemcpyHostToDevice, stream));
    }

    // Upload existing grad_in for accumulation
    float* d_grad_in = g_buf_cache.get(total);
    float* p_grad_in = g_pinned_cache.get(total);
    memcpy(p_grad_in, h_grad_in, total * sizeof(float));
    CUDA_CHECK(cudaMemcpyAsync(d_grad_in, p_grad_in, total * sizeof(float),
                                cudaMemcpyHostToDevice, stream));

    // Launch kernel
    global_avgpool_bwd_k<<<grid_for((int)total), kBlock, 0, stream>>>(
        d_grad_out, d_grad_in, (int)total, (int)spatial);

    // D2H
    CUDA_CHECK(cudaMemcpyAsync(p_grad_in, d_grad_in, total * sizeof(float),
                                cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    memcpy(h_grad_in, p_grad_in, total * sizeof(float));

    // Return buffers
    g_pinned_cache.put(p_grad_in, total);
    if (p_grad_out) g_pinned_cache.put(p_grad_out, bc);

    g_buf_cache.put(d_grad_in, total);
    if (!grad_out_shadow) g_buf_cache.put(d_grad_out, bc);
}

// ---------------------------------------------------------------------------
// Host-pointer elementwise accumulate: target[i] += source[i]
// Does NOT register shadow for target (gradient being accumulated in backward).
// ---------------------------------------------------------------------------
void CUDABackend::elementwise_accumulate_host(float* h_target, const float* h_source, size_t n) {
    init();
    if (!initialized_ || n == 0) return;

    cudaStream_t stream = static_cast<cudaStream_t>(stream_);

    // Check shadow cache for source
    float* d_source = g_shadow_cache.find(h_source);
    bool source_shadow = (d_source != nullptr);

    float* p_source = nullptr;

    if (!source_shadow) {
        d_source = g_buf_cache.get(n);
        p_source = g_pinned_cache.get(n);
        memcpy(p_source, h_source, n * sizeof(float));
        CUDA_CHECK(cudaMemcpyAsync(d_source, p_source, n * sizeof(float),
                                    cudaMemcpyHostToDevice, stream));
    }

    // Upload existing target for accumulation
    float* d_target = g_buf_cache.get(n);
    float* p_target = g_pinned_cache.get(n);
    memcpy(p_target, h_target, n * sizeof(float));
    CUDA_CHECK(cudaMemcpyAsync(d_target, p_target, n * sizeof(float),
                                cudaMemcpyHostToDevice, stream));

    // Launch kernel
    accumulate_k<<<grid_for((int)n), kBlock, 0, stream>>>(d_target, d_source, (int)n);

    // D2H
    CUDA_CHECK(cudaMemcpyAsync(p_target, d_target, n * sizeof(float),
                                cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));
    memcpy(h_target, p_target, n * sizeof(float));

    // Return buffers — do NOT register shadow for target
    g_pinned_cache.put(p_target, n);
    if (p_source) g_pinned_cache.put(p_source, n);

    g_buf_cache.put(d_target, n);
    if (!source_shadow) g_buf_cache.put(d_source, n);
}
