import boto3
from typing import BinaryIO
from services.credential_service import CredentialService


def _client_kwargs(
    access_key: str,
    secret_key: str,
    region: str,
    endpoint_url: str | None,
) -> dict:
    kwargs = {
        "aws_access_key_id": access_key,
        "aws_secret_access_key": secret_key,
        "region_name": region or "us-east-1",
    }
    if endpoint_url:
        kwargs["endpoint_url"] = endpoint_url
    return kwargs


class S3Service:
    def __init__(self, cred_service: CredentialService | None = None):
        self.cred_service = cred_service or CredentialService()

    def _get_client(
        self,
        encrypted_access_key: bytes,
        encrypted_secret_key: bytes,
        region: str,
        endpoint_url: str | None = None,
    ):
        return boto3.client(
            "s3",
            **_client_kwargs(
                self.cred_service.decrypt(encrypted_access_key),
                self.cred_service.decrypt(encrypted_secret_key),
                region,
                endpoint_url,
            ),
        )

    def list_buckets(
        self,
        encrypted_ak: bytes,
        encrypted_sk: bytes,
        region: str,
        endpoint_url: str | None = None,
    ) -> list[dict]:
        client = self._get_client(encrypted_ak, encrypted_sk, region, endpoint_url)
        resp = client.list_buckets()
        return [{"name": b["Name"], "created": b["CreationDate"].isoformat()} for b in resp["Buckets"]]

    def create_bucket(
        self,
        encrypted_ak: bytes,
        encrypted_sk: bytes,
        region: str,
        bucket_name: str,
        endpoint_url: str | None = None,
    ):
        client = self._get_client(encrypted_ak, encrypted_sk, region, endpoint_url)
        create_args: dict = {"Bucket": bucket_name}
        if not endpoint_url:
            config = {"LocationConstraint": region} if region != "us-east-1" else {}
            if config:
                create_args["CreateBucketConfiguration"] = config
        client.create_bucket(**create_args)

    def list_objects(
        self,
        encrypted_ak: bytes,
        encrypted_sk: bytes,
        region: str,
        bucket: str,
        prefix: str = "",
        endpoint_url: str | None = None,
    ) -> list[dict]:
        client = self._get_client(encrypted_ak, encrypted_sk, region, endpoint_url)
        resp = client.list_objects_v2(Bucket=bucket, Prefix=prefix, Delimiter="/")
        items = []
        for p in resp.get("CommonPrefixes", []):
            items.append({"key": p["Prefix"], "type": "folder"})
        for obj in resp.get("Contents", []):
            items.append({
                "key": obj["Key"],
                "type": "file",
                "size": obj["Size"],
                "last_modified": obj["LastModified"].isoformat(),
            })
        return items

    def upload_file(
        self,
        encrypted_ak: bytes,
        encrypted_sk: bytes,
        region: str,
        bucket: str,
        key: str,
        file_obj: BinaryIO,
        endpoint_url: str | None = None,
    ):
        client = self._get_client(encrypted_ak, encrypted_sk, region, endpoint_url)
        client.upload_fileobj(file_obj, bucket, key)

    def delete_object(
        self,
        encrypted_ak: bytes,
        encrypted_sk: bytes,
        region: str,
        bucket: str,
        key: str,
        endpoint_url: str | None = None,
    ):
        client = self._get_client(encrypted_ak, encrypted_sk, region, endpoint_url)
        client.delete_object(Bucket=bucket, Key=key)

    def generate_presigned_url(
        self,
        encrypted_ak: bytes,
        encrypted_sk: bytes,
        region: str,
        bucket: str,
        key: str,
        expires_in: int = 3600,
        endpoint_url: str | None = None,
    ) -> str:
        client = self._get_client(encrypted_ak, encrypted_sk, region, endpoint_url)
        return client.generate_presigned_url(
            "get_object", Params={"Bucket": bucket, "Key": key}, ExpiresIn=expires_in
        )
